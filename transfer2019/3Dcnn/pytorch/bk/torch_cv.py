...
parser = argparse.ArgumentParser(description='PyTorch for AD, usage example: python3 torch_cv.py /tmp/ad/output /tmp/ad/MRI_folder /tmp/ad/lable.txt')
...
parser.add_argument('--lr', '-lr', type=float, default=0.01, help='Learning Rate, default=0.001')
...
parser.add_argument('outputFolder', default='', type=str, help='The output folder. eg. /tmp/folder')
parser.add_argument('MRIdataFolder', type=str, help='MRI images folder. eg. /tmp/folder')
parser.add_argument('labelFile',  type=str, help='Label file. eg. /tmp/folder/lable.txt')
...
class LogSystem:
    def __init__(self, log_folder, test_flag=False):
        self.test = test_flag
        self.sep = '_'
        log_filename = opt.MRIdataFolder + '_' + self.sep.join(comparison) + '_' + time.strftime("%Y_%m_%d_%H_%M",
                                                                                           time.localtime()) + '.txt'
        if not self.test:
...
def layers(config, width, height, depth):
    ...
    # width, height, depth, nLabel
    # h = int(((((((self.height - 3) + 1) // 2 - 3 + 1) // 2 - 3) // 2 + 1) // 2 - 3 + 1) // 2)
    for v in config:
        if v == 'M':
...
cfg = {
    ...
    'B': [{'out_channel': 32, 'size': 3, 'stride': 1, 'pad': 0}, 'R', 'M',
          {'out_channel': 32, 'size': 3, 'stride': 1, 'pad': 0}, 'R', 'M',
          {'out_channel': 32, 'size': 3, 'stride': 1, 'pad': 0}, 'R', 'M',
          {'out_channel': 32, 'size': 3, 'stride': 1, 'pad': 0}, 'R', 'M'],
}
...
if __name__ == "__main__":
    ...
    while cv < opt.crossValidation:
        ...
        model = CNN(*layers(cfg['A'], sample_depth, sample_height, sample_width), nLabel)
