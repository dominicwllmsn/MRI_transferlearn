import sys, subprocess, threading, time, queue, os, re
...
def fslthread(threadname, filepath):
    ...
    if filepath == '':
        ...
        if m == None:
            ...
            f.write(
                "flirt" + " -in " + brainFilePath + " -ref " + MNI152brain_1mm + " -out " + affineFilePath + ".nii.gz" + " -omat " + affineFilePath + ".mat" + " -bins 256 -cost corratio -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 12  -interp trilinear" + "\n")
            f.write(str(subprocess.check_output(
...
Test = False
# path of ROBEX
ROBEXFolder = "/media/chao/claudiohome/Toolkit/ROBEX/"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 5 or Test :
        if Test:
            ...
            fileFolderPath = '/media/chao/claudiolocal/ADNI_Screening/'
            # MASK file path
            ROImaskFilename = '/media/chao/claudiolocal/sys/AtlasROIMask/harvardoxford_prob_Hippo_Amyg_Thal_Puta_Pall_Caud_MTL_ROI_Mask_1mm_test.nii.gz'
            # output folder path
            OutputDir = '/media/chao/claudiolocal/sys/output/o2/'
        else:
            nthreads = int(sys.argv[1])
            fileFolderPath = sys.argv[2]
            ROImaskFilename = sys.argv[3]
            OutputDir = sys.argv[4]

    else:
        print ("Usage:")
        print ("python3 preprocess.py <#Threads> <Files Folder Path> <ROI Mask File> <Output Path>")
        exit()
