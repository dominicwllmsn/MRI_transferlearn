...
ROIList_Full = getFilesinPathROI2(ROI_path)
...
with open('./over480_type.txt', 'w+') as f_out:
    for line in ROIList_Full:
        ind_stop = line.index('4_ROI_1mm') - 1
        line_short = line[:ind_stop].replace(ROI_path+'/', '')[11:]
        for i,nii in enumerate(niiList):
            if line_short+'.nii' in nii & os.path.getsize(line) > 480000:
                print(nii)
